# 🤖 Android & Windows Gemini Automation Agent
...
[![Platforms](https://img.shields.io/badge/Platforms-Android%20%7C%20Windows-0078D6?logo=windows&logoColor=white)](https://github.com/)
[![Tests Passing](https://img.shields.io/badge/Tests-255%2F255%20Passed-brightgreen.svg?logo=pytest&logoColor=white)](tests/)
[![CI Status](https://img.shields.io/badge/CI-Passing-brightgreen?logo=github-actions&logoColor=white)](.github/workflows/ci.yml)
...
The **Android & Windows Gemini Automation Agent** takes a radically faster, cheaper, and more accurate approach:

1. **Native Accessibility Extraction**: Directly inspects the native OS accessibility trees (**Android UI Automator XML** via Wi-Fi ADB or **Windows UI Automation** via COM).
...
5. **Native OS Execution**: Injects actions instantly via low-latency native drivers (`input tap/swipe/text` on Android, `pyautogui`/COM on Windows).

> 🚀 **Result**: **Sub-second decision cycles**, **80–90% token reduction**, **zero visual grounding hallucinations**, and **24/7 sustainable operation** within standard free-tier quotas.
...
<table>
  ...
  <tr>
    ...
    <td width="50%">
      ...
      <p>Automatic 429 exponential backoff retry engine paired with a dynamic model fallback cascade (<code>3.5-flash-lite</code> &rarr; <code>3.1-flash-lite</code> &rarr; <code>2.5-flash-lite</code> &rarr; <code>2.5-flash</code>).</p>
    </td>
  </tr>
  <tr>
    <td width="50%">
    ...
    <td width="50%">
      ...
      <p>Beautiful terminal UI with live reasoning spinners, syntax-highlighted step cards, tabular screen inspector, and safe <code>Ctrl+C</code> interruption.</p>
    </td>
  </tr>
</table>
...
│  • UiAutomator XML Dump & AST Pruner      │   │  • Unicode & Emoji Clipboard Injection  │
│  • Shell Escaping & Non-ASCII Fallback    │   │  • Win+R / Executable Launching Cascade │
└───────────────────────────────────────────┘   └─────────────────────────────────────────┘
```
...
| Metric / Dimension | Visionless Fast-Path (This Agent) | Multimodal VLM (Screenshot-Based) |
|---|---|---|
...
| **Turn Latency** | **400ms – 1,500ms** (Instant inference) | **2,500ms – 6,000ms** (Image upload + tokenization) |
...
git clone https://github.com/android-gemini-agent/android-gemini-agent.git
...
android-gemini --platform windows

# Or run a single desktop task directly
android-gemini --platform windows --task "Open Notepad, type 'Hello from Gemini!', and save to Desktop"
```

> 💡 **Tip**: When running inside the REPL, switch between platforms on the fly anytime by typing `platform windows` or `platform android`.
...
| `status` | None | Displays connection state, Gemini model, API key, and safety limits. | `status` |
...
android-gemini --task "Open Clock, set timer for 5 minutes, and start"

# Execute a single task on Windows Desktop
android-gemini --platform windows --task "Open Calculator and compute 256 * 16"

# Pair an Android device over Wi-Fi
android-gemini --pair 192.168.1.45:38912 654321

# Connect to device and inspect current UI elements
android-gemini --connect 192.168.1.45:41253 --dump-ui

# Inspect status and verify API connectivity
android-gemini --status

# Override the Gemini model identifier
android-gemini --model gemini-3.5-flash-lite --task "Search for nearby coffee shops in Maps"
```
...
2. **Automatic 429 Backoff**: If rate limits are encountered, the engine intercepts the exception, parses the server `retryDelay` or applies progressive exponential backoff ($5.0 \times \text{attempt}$ seconds), and retries transparently.
3. **Model Fallback Cascade**: If the primary model encounters severe capacity exhaustion, the engine automatically attempts the request against secondary lite models before reporting an error.
...
- **Recovery Prompt Injection**: Injects targeted guidance instructing Gemini to re-evaluate visible IDs, swipe to reveal hidden content, or backtrack. If the deadlock persists for $>2$ consecutive cycles, execution is aborted safely to prevent wasted tokens.
...
4. Tap <i>Pair device with pairing code</i> and run <code>android-gemini --pair <ip:port> <code></code>.
</details>
...
<b>Fix:</b>
1. Increase <code>ACTION_DELAY_SECONDS=2.0</code> in <code>.env</code>.
2. The agent automatically executes exponential backoff and model cascade retries.
...
- **Questions?** Join the discussion in [GitHub Discussions](https://github.com/android-gemini-agent/android-gemini-agent/discussions).
